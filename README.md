...
M8 的第一刀增加了可选的语音转文字错字救援骨架：可信本地调用方可注入一份
有上限、可重建的查询映射与保护词表；首轮为空或结果既没支撑疑似错词、也没直接
找到目标词时才补查，
名字、昵称和方言保护词不会被替换。原查询、触发原因、候选轮次和最终选中记录
都会进入独立审计轨迹，记录、Projection 与 Claim 本身不改写。公共仓库只保留
机制和纯合成验收，不保存私人词表；完整边界见
[`docs/SPEECH_QUERY_RESCUE.md`](docs/SPEECH_QUERY_RESCUE.md)。
