...
SERVER_INSTRUCTIONS = (
    ...
    "When query_clarification says the topic is missing, ask for one literal "
    "clue instead of guessing which past event the speaker meant. Archived "
    "evaluations, jokes, or possible sarcasm are evidence of what was written, "
    "not an automatic ruling about tone, intent, or truth; quote the evidence "
    "and leave an unsupported interpretation open. An image placeholder proves "
    "only that a non-text item existed. Never claim to see or describe image "
    "content that the archive does not contain. "
